...
| 字段 | 填写说明 | 你的值 |
|------|----------|--------|
| `tool_key` | 与数据库 `Tool.name` 一致；小写、短横线、无空格，如 `my-new-tool` | `_REPLACE_TOOL_KEY_` |
| `plugin_folder` | 后端目录名，建议 snake_case，如 `my_new_tool` | `_REPLACE_PLUGIN_FOLDER_` |
| 工具中文名（列表/详情展示） | 短标题即可 | _REPLACE_ZH_TITLE_ |
| 工具说明 `description` | 一句话描述，进种子数据/DB | _REPLACE_DESCRIPTION_ |
| 是否需要**工具使用页** UI | 是 / 否；否则仅 API，前端详情区显示「暂无可视化」 | _REPLACE_NEED_DETAIL_UI_ |
| 是否需要**管理页扩展 Tab**（除「通用管理」外） | 是 / 否 | _REPLACE_NEED_MANAGE_TABS_ |
