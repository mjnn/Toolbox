...
新增网络约束：除部署机外，其他用户机无法直接访问外网，因此工具页默认不能依赖公网直播地址直连。应由部署机先将直播画面转发到内网地址（如 Nginx/反向代理/转发服务），工具管理页只维护“内网展示地址”，用户侧统一访问该内网地址。

### 2.2 约束与假设（可选）

（非目标、合规、仅内网、账号由谁提供、必须中文文案、性能大致预期等；无则写「无」）

- 非目标：不在浏览器端直接访问公网直播地址。
- 网络策略：部署机可访问外网，普通用户机仅可访问内网。
- 运维约束：负责人需在工具管理中维护“内网直播展示地址”（部署机转发后的地址）。
- 文案要求：页面提示与错误信息必须中文。
...
|------|------|----------|----------------------------------------|
| v0.1 | 2026-4-21 | 初稿 | |
| v0.2 | 2026-4-21 | 增加内网访问约束与部署机转发方案 | 工具管理页统一维护“内网直播展示地址”；用户页不再默认依赖公网直连 |
| | | （后续修订继续追加行） | |
...
### 2.5 部署机转发落地建议（运维执行参考）

目标：**部署机可访问外网直播源，用户机仅访问内网地址**。工具管理页只维护内网展示地址。

方案 A（推荐，简单稳定）：部署机 Nginx 反向代理外网 FLV

- 部署机提供内网地址，例如：`http://10.10.10.20/live/livestream.flv`
- Nginx 示例（仅示意）：
  - `location /live/ { proxy_pass http://47.116.180.173:8080/live/; proxy_buffering off; }`
- 工具管理页将“内网直播展示地址”配置为该内网 URL（或对应播放器页 URL）。

方案 B：部署机本地 SRS/Nginx-RTMP 二次中转

- 部署机先拉流（或接 OBS 推流）到本地 SRS，再由 SRS 对内提供 HTTP-FLV。
- 优点：可做更多流控（并发、重试、录制、鉴权）；缺点：运维复杂度更高。

方案 C：部署机 FFmpeg 拉流转发到本地 HTTP-FLV 服务

- 用 FFmpeg 持续拉取外网源，再推到部署机本地流媒体服务，由内网用户访问本地服务。
- 适合临时方案或 PoC，不建议长期裸用单进程转发。

运维验收清单（建议）

- 部署机可访问外网源，用户机不可访问外网源（符合隔离预期）。
- 用户机可访问“内网直播展示地址”并稳定播放。
- 工具管理页配置更新后，工具详情页可立即生效。
- 外网源中断时，工具页可切换占位页并提示中文文案。
