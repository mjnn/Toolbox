...
车规对数据安全要求严格，功能研发阶段每个功能都涉及到数据的使用，数据安全负责人需要对所有的功能进行数据安全相关的操作包括：相关性判定，数据字段收集，数据分类分级，数据安全要求等

工具能力：

该工具需要具备如下能力：
工具的负责人为：数据安全FO (Function Owner)；工具的用户为：功能FO；
支持按照不同的项目空间进行隔离，数据安全FO可以在工具管理中，新增项目空间，后续的所有的指定规则，制定模板等操作，都是各个项目独立的
对下面操作进行流程性的实现（类似工单流转）

数据安全相关性判定：
数据安全FO在 工具管理 中制定问卷和判定规则
功能FO进入对应项目空间之后填写自己是什么功能然后开始填写问卷
工具根据 判定规则 判断功能和数据安全是否相关，把结果反馈给功能FO，记录填报记录和结果反馈给数据安全FO；
功能如果不相关则仅记录填报记录即可，如果相干，则进入流程下一环节。

数据字段填报：
数据安全FO在工具管理中制定数据全生命周期表，数据字段是其中一列（唯一值索引），其他每个列的字段（表头）可以由数据安全FO自由的增加减少以及增加字段限制和提醒（可以复用字段配置组件）
功能FO从已有的数据字段中下拉选择（支持模糊搜索）自己功能涉及的数据字段，并且填写改数据涉及的其他字段，如功能FO找不到涉及到的数据字段，则可提交新增字段申请。数据安全FO在收到功能FO提交的新增字段申请后审核是否加入数据字段表；可以按需导出功能涉及的数据字段信息

数据分类分级：

数据安全FO在工具管理中制定数据分类分级标准表
系统收到用户填报数据字段之后，自动对数据字段进行分类分级，支持数据安全FO更新数据分类分级标准，编辑和检查分类分级结果；功能FO可以只读的查阅分类分级结果

数据安全要求：

自动依据分级生成每个数据的数据安全要求；数据安全FO可以检查（读）更新（写）数据安全要求并按功能维度导出数据安全要求；功能FO只能查阅自己功能的数据安全要求
