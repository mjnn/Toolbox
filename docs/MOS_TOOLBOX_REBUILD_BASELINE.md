...
> **适用范围**：本文档描述的是 **从旧 `ref/toolboxweb` 迁入 MOS 综合工具箱时** 的能力裁剪与「**一个 `tool_id`、多 Tab**」的呈现约定（历史重构基线）。**不等于**整个平台只支持单工具；仓库仍支持多 `Tool` 行与多插件并存。
>
> 状态：已确认（用户选择：**上述迁移范围下的单工具呈现** + **UAT/LIVE 合并页环境切换**）
...
- 工具形态（**本迁移范围内**）：**单工具**（一个 `tool_id`，多能力以 Tab 聚合）
