...
$PidFile = Join-Path $Root "run\toolbox-frontend.pid"

if (-not (Test-Path $PidFile)) {
    Write-Host "No frontend pid file. Caddy may already be stopped."
    exit 0
}
...
if ($proc) {
    ...
    Write-Host "Stopped Caddy frontend (PID: $targetPid)."
} else {
