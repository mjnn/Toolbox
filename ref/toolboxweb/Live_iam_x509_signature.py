import json
import re
from Live_Zone_token_getter import *
...
def live_iam_x509_cert_sign(csr, zone_token, cookies):
    csr = csr_preprocess(csr)
    url = 'https://zapi-zone.mosop.csvw.com/usap/web/webapi/x509/carCert/apply'
    payload = {
        'csr': csr
    }
    apply_response = post_request_to_zone_live_usap(url, payload, zone_token, cookies)
    ...
    if apply_result != '成功':
        ...
        url = f'https://zapi-zone.mosop.csvw.com/usap/web/webapi/x509/carCert/search?perPage=10&page=1&__rid={str(decimal_random)}'
        search_cert_response = get_request_to_zone_live_usap(url, zone_token, cookies)
        ...
        if cert_id == '':
            ...
            url = f'https://zapi-zone.mosop.csvw.com/usap/web/webapi/x509/cert/export?certId={cert_id}'
            export_cert_response = get_request_to_zone_live_usap(url, zone_token, cookies)
...
def live_get_x509_cert_by_sn(sn, zone_token, cookies):
    decimal_random = generate_17_decimal_random()
    url = f'https://zapi-zone.mosop.csvw.com/usap/web/webapi/x509/carCert/search?sn={sn}&perPage=10&page=1&__rid={str(decimal_random)}'
    search_cert_response = get_request_to_zone_live_usap(url, zone_token, cookies)
    certs = json.loads(search_cert_response.text)['data']['items']
    if not certs:
